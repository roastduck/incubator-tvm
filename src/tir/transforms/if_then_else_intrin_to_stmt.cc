...
class SubExprVisitor : public ExprVisitor {
 ...
 protected:
  explicit SubExprVisitor(const RetType& ret) : ret_(ret) {}
...
class SubExprMutator : public ExprMutator {
 public:
  ...
  Stmt Mutate(const ForNode* op) {
    ...
    return ForNode::make(op->loop_var, min, extent, op->for_type, op->device_api, op->body);
  }
  ...
  Stmt Mutate(const AllocateNode* op) {
    ...
    auto condition = operator()(std::move(op->condition));
    return AllocateNode::make(op->buffer_var, op->dtype, extents, condition, op->body);
  }
  ...
  Stmt Mutate(const RealizeNode* op) {
    auto condition = operator()(std::move(op->condition));
    return RealizeNode::make(op->func, op->value_index, op->dtype, op->bounds, condition, op->body);
  }
 ...
 * 4. Put the statement into a new IfThenElse statement, and remove the out-most tvm_if_then_else
 * intrinsic.
...
class IfThenElseReplacer : public StmtMutator {
 ...
 protected:
  ...
  Stmt VisitStmt_(const IfThenElseNode* op) override { return Replace_(op); }

  Stmt VisitStmt_(const LetStmtNode* op) override { return Replace_(op); }

  Stmt VisitStmt_(const ForNode* op) override { return Replace_(op); }

  Stmt VisitStmt_(const AllocateNode* op) override { return Replace_(op); }

  Stmt VisitStmt_(const StoreNode* op) override { return Replace_(op); }

  Stmt VisitStmt_(const ProvideNode* op) override { return Replace_(op); }

  Stmt VisitStmt_(const RealizeNode* op) override { return Replace_(op); }

  Stmt VisitStmt_(const EvaluateNode* op) override { return Replace_(op); }
 ...
 private:
  ...
  Stmt Replace_(const Node* op) {
    ...
    if (cnt > 0 && cnt <= max_cascading_intrin_) {
      ...
      Stmt ret = IfThenElseNode::make(condition, Downcast<Stmt>(true_branch),
                                      Downcast<Stmt>(false_branch));
...
TVM_REGISTER_GLOBAL("tir.transform.IfThenElseIntrinToStmt").set_body_typed(IfThenElseIntrinToStmt);
